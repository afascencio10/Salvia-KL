...
// FeminicidePOST maneja la solicitud POST para crear o actualizar un caso de víctima.
...
func FeminicidePOST(c *gin.Context) {
	...
	// Verifica que el usuario tenga el permiso "set_feminicide".
	if !utils.CheckPermission(salvia_config.PermissionsByRole, "set_feminicide", s.CurrentRole, c) {
		return
	}
	...
	code, res := salvia_ctrl.SetFeminicide(buf.String(), *s, dbClientConfig, dbServerConfig)
	// Envía la respuesta al cliente en formato JSON.
	c.DataFromReader(code, int64(len(res)), gin.MIMEJSON, strings.NewReader(res), nil)
}

// FeminicideGET maneja la solicitud GET para obtener casos de víctima.
...
func FeminicideGET(c *gin.Context) {
	...
	if id != "" {
		// Si no se especifica "docType", se verifica el permiso "get_feminicide".
		if !utils.CheckPermission(salvia_config.PermissionsByRole, "get_feminicide", s.CurrentRole, c) {
			return
		}
		// Se obtiene el caso por su código interno.
		code, feminicideRes, _ = salvia_ctrl.GetFeminicideByICode(id, &db.ConnData{}, dbClientConfig, dbServerConfig)

	} else {
		//Si se consultan todos los casos
		// Si no se proporciona un ID, se verifica el permiso "get_feminicides" para obtener todos los casos.
		if !utils.CheckPermission(salvia_config.PermissionsByRole, "get_feminicides", s.CurrentRole, c) {
			return
		}
		code, feminicideRes, count = salvia_ctrl.GetFeminicideByAll(page, &db.ConnData{}, dbClientConfig, dbServerConfig)
	}
	...
	if v, found := c.Request.Header["Accept"]; found && v[0] == "application/json" {
		...
		common_facades.RenderTemplate(c, salvia_daos.VictimContactEntityName, "salvia", "feminicide/", salvia_config.HTML_Templates, tplName, utils.GetFullHtmlTemplates(), utils.DEFAULT_VIEW, utils.DEFAULT_PANIC_TEMPLATE,
			map[string]interface{}{
				"windowTitle": salvia_config.Locale["sp"]["get_feminicide_window_title"],
				"currentUser": s.Names + " " + s.LastNames,
				"nav_rules":   salvia_config.TranslateNavigationRule(s.Lang, salvia_config.NAVIGATION_RULES["get_feminicide"]),
				"locale":      salvia_config.Locale,
				"lang":        s.Lang,
				"feminicides": feminicideRes,
				"numPages":    int(math.Ceil(float64(count) / float64(salvia_config.NUM_ITEMS_PER_PAGE))),

				"departments":              departments,
				"menu":                     menu,
				"menuToolsFeminicideTable": feminicideMenuTools,
				"momentCodes":              salvia_config.MOMENT[s.Lang],
				"sectors":                  salvia_config.SECTOR[s.Lang],
				"salviaFeminicideFormPath": salvia_config.FormPaths[s.Lang]["FeminicideGET"],
			}, utils.GetFullHtmlFuncMap())
	}
}

// FeminicideReportGET maneja la solicitud GET para la generación de reportes de casos de víctimas.
// Recupera información necesaria, como los departamentos, y renderiza la plantilla correspondiente.
func FeminicideReportGET(c *gin.Context) {
	...
	common_facades.RenderTemplate(c, salvia_daos.VictimContactEntityName, "salvia", "feminicide/", salvia_config.HTML_Templates, "report_feminicides", utils.GetFullHtmlTemplates(), utils.REPORT_VIEW, utils.DEFAULT_PANIC_TEMPLATE,
		map[string]interface{}{
			"windowTitle":          salvia_config.Locale["sp"]["report_feminicide_window_title"],
			"currentUser":          s.Names + " " + s.LastNames,
			"nav_rules":            salvia_config.TranslateNavigationRule(s.Lang, salvia_config.NAVIGATION_RULES["report_feminicides"]),
			"locale":               salvia_config.Locale,
			"lang":                 s.Lang,
			"menu":                 menu,
			"departments":          departments,
			...
			"salviaReportFormPath": salvia_config.FormPaths[s.Lang]["FeminicideReportGET"],
		}, utils.GetFullHtmlFuncMap())
}

// FeminicideReportPOST maneja la solicitud POST para generar reportes de casos de víctimas.
// Verifica permisos, procesa los datos recibidos, y renderiza la respuesta ya sea en JSON o mediante plantilla HTML.
func FeminicideReportPOST(c *gin.Context) {
	// Se obtiene la sesión y se extrae el ID de sesión.
	session := sessions.Default(c)
	var sessionID string = session.Get("userData").(string)
	s, err := utils.GetCommonSession(sessionID)
	...
	var feminicidesRes string
	var code int = http.StatusOK
	var tplName string = "report_feminicides"
	// Verifica que el usuario tenga el permiso "report_feminicides".
	if !utils.CheckPermission(salvia_config.PermissionsByRole, "report_feminicides", s.CurrentRole, c) {
		return
	}
	...
	code, feminicidesRes = salvia_ctrl.SetFeminicide(buf.String(), *s, dbClientConfig, dbServerConfig)

	// Según el header "Accept", se devuelve la respuesta en JSON o se renderiza la plantilla.
	if v, found := c.Request.Header["Accept"]; found && v[0] == "application/json" {
		c.DataFromReader(code, int64(len(feminicidesRes)), gin.MIMEJSON, strings.NewReader(feminicidesRes), nil)
	} else {
		var menu map[string][]map[string]string
		...
		common_facades.RenderTemplate(c, salvia_daos.VictimContactEntityName, "salvia", "feminicide/", salvia_config.HTML_Templates, tplName, utils.GetFullHtmlTemplates(), utils.REPORT_VIEW, utils.DEFAULT_PANIC_TEMPLATE,
			map[string]interface{}{
				"windowTitle":          salvia_config.Locale["sp"]["report_feminicide_window_title"],
				"currentUser":          s.Names + " " + s.LastNames,
				"nav_rules":            salvia_config.TranslateNavigationRule(s.Lang, salvia_config.NAVIGATION_RULES["report_feminicides"]),
...
// FeminicidePUT_GET maneja la solicitud GET para obtener la información de un caso de víctima a actualizar.
// Recupera datos relacionados con el caso, contacto, ubicación y entidades, y renderiza la plantilla de actualización.
/*func FeminicidePUT_GET(c *gin.Context) {
	// Se obtiene la sesión y se extrae el ID de sesión.
	session := sessions.Default(c)
	var sessionID string = session.Get("userData").(string)
	s, _ := utils.GetCommonSession(sessionID)

	// Verifica que el usuario tenga el permiso "update_feminicide".
	if !utils.CheckPermission(salvia_config.PermissionsByRole, "update_feminicide", s.CurrentRole, c) {
		return
	}
	...
	if id != "" {
		...
		_, _, feminicide = salvia_ctrl.GetFeminicideByICode(id, &db.ConnData{}, dbClientConfig, dbServerConfig)
		_, _, victimContact = salvia_ctrl.GetVictimContactByICode(strconv.FormatUint(feminicide.FeminicideVictimContact.VictimContactId, 10), &db.ConnData{}, dbClientConfig, dbServerConfig)
	}

	// Carga datos adicionales en feminicide, excluyendo información redundante del contacto.
	feminicide.LoadFromWithoutVictimContact(salvia_config.MOMENT[s.Lang], salvia_config.SECTOR[s.Lang], entities)

	// Para el lugar de los hechos, se obtienen las ciudades y pueblos basados en el departamento y ciudad seleccionados.
	if feminicide.FeminicideForm2.FeminicideForm2FactsDepartment.DepartmentId > 0 {
		_, cities2, _ = security_ctrl.GetCitiesByDeparment(feminicide.FeminicideDepartment.DepartmentId, &db.ConnData{}, dbClientConfig, dbServerConfig)
	}
	if feminicide.FeminicideTown.TownId > 0 {
		_, towns2 = security_ctrl.GetTownsByCity(feminicide.FeminicideCity.CityId, &db.ConnData{}, dbClientConfig, dbServerConfig)
	}

	// Para el lugar de atención, se obtienen las ciudades y pueblos correspondientes.
	if feminicide.FeminicideDepartment.DepartmentId > 0 {
		_, cities, _ = security_ctrl.GetCitiesByDeparment(feminicide.FeminicideDepartment.DepartmentId, &db.ConnData{}, dbClientConfig, dbServerConfig)
	}
	if feminicide.FeminicideTown.TownId > 0 {
		_, towns = security_ctrl.GetTownsByCity(feminicide.FeminicideCity.CityId, &db.ConnData{}, dbClientConfig, dbServerConfig)

		// Se obtienen las ramas de entidad asociadas al pueblo para asignar la ruta.
		var branches []salvia_daos.EntityBranchDTO
		_, _, branches = salvia_ctrl.GetEntityBranchesByTownCodeWithMoments(feminicide.FeminicideTown.TownCode, &db.ConnData{}, dbClientConfig, dbServerConfig)
	...
	common_facades.RenderTemplate(c, salvia_daos.FeminicideEntityName, "salvia", "feminicide/", salvia_config.HTML_Templates, "update_feminicide", utils.GetFullHtmlTemplates(), utils.DEFAULT_VIEW, utils.DEFAULT_PANIC_TEMPLATE,
		map[string]interface{}{
			"windowTitle":               salvia_config.Locale["sp"]["update_feminicide_window_title"],
			"currentUser":               s.Names + " " + s.LastNames,
			...
			"feminicideVictimNationality":                         salvia_config.VICTIM_CASE_VICTIM_NATIONALITY,
			"feminicideVictimForeignerImmigrationStatus":          salvia_config.VICTIM_CASE_VICTIM_FOREIGNER_IMMIGRATION_STATUS,
			"feminicideVictimGender":                              salvia_config.VICTIM_CASE_VICTIM_GENDER,
			"feminicideVictimDependents":                          salvia_config.VICTIM_CASE_VICTIM_DEPENDENTS,
			"feminicideVictimDeathThreats":                        common_config.YES_NO,
			"feminicideVictimAggressorHasWeapons":                 common_config.YES_NO,
			"feminicideExperiencedPhysicalOrSexualViolenceBefore": common_config.YES_NO,
			"feminicideVictimImminentRisk":                        common_config.YES_NO,
			"feminicideVictimPreviouslyReportedSituation":         common_config.YES_NO,
			"feminicideVictimIfPreviouslyReported":                salvia_config.VICTIM_CASE_VICTIM_IF_PREVIOUSLY_REPORTED,
			...
			"salviaFormPath":             salvia_config.FormPaths[s.Lang]["FeminicidePUT"],
			"securityCityFormPath":       security_config.FormPaths[s.Lang]["CityGET"],
			...
			"nav_rules":                  salvia_config.TranslateNavigationRule(s.Lang, salvia_config.NAVIGATION_RULES["update_feminicide"]),
		}, utils.GetFullHtmlFuncMap())
}*/

// FeminicidePOST_GET maneja la solicitud GET para inicializar la creación de un caso de víctima.
...
func FeminicidePOST_GET(c *gin.Context) {
	...
	common_facades.RenderTemplate(c, salvia_daos.FeminicideEntityName, "salvia", "feminicide/", salvia_config.HTML_Templates, "set_feminicide", utils.GetFullHtmlTemplates(), utils.DEFAULT_VIEW, utils.DEFAULT_PANIC_TEMPLATE,
		map[string]interface{}{
			"windowTitle": salvia_config.Locale["sp"]["set_feminicide_window_title"],
			"currentUser": s.Names + " " + s.LastNames,
			"gender":      common_config.GENDER_IDENTITY,
			"language":    common_config.LANGUAGE,
			"locale":      salvia_config.Locale,
			"lang":        s.Lang,
			"livingZone":  salvia_config.LIVING_ZONES,
			"feminicide":  salvia_daos.FeminicideDTO{},
			...
			"docType":            salvia_daos.VictimCaseForm2Enums["feminicide_form2_victim_doc_type"],
			"factsZone":          salvia_daos.VictimCaseForm2Enums["feminicide_form2_facts_zone"],
			"migrationCondition": salvia_daos.VictimCaseForm2Enums["feminicide_form2_migration_condition"],
			"genderIdentity":     salvia_daos.VictimCaseForm2Enums["feminicide_form2_gender_identity"],
			"sexualOrientation":  salvia_daos.VictimCaseForm2Enums["feminicide_form2_sexual_orientation"],
			"ethnicAffiliation":  salvia_daos.VictimCaseForm2Enums["feminicide_form2_ethnic_affiliation"],
			"indigenousPeople":   salvia_daos.VictimCaseForm2Enums["feminicide_form2_indigenous_people"],
			"maritalStatus":      salvia_daos.VictimCaseForm2Enums["feminicide_form2_marital_status"],
			"lastEducationLevel": salvia_daos.VictimCaseForm2Enums["feminicide_form2_last_education_level"],

			//Campos múltiples

			"moments":                    salvia_config.MOMENT[s.Lang],
			"sectors":                    salvia_config.SECTOR[s.Lang],
			"salviaFormPath":             salvia_config.FormPaths[s.Lang]["FeminicidePOST_GET"],
			"securityCityFormPath":       security_config.FormPaths[s.Lang]["CityGET"],
			"salviaEntityBranchFormPath": salvia_config.FormPaths[s.Lang]["EntityBranchGET"],
			"securityTownFormPath":       security_config.FormPaths[s.Lang]["TownGET"],
			"departments":                departments,
			"nav_rules":                  salvia_config.TranslateNavigationRule(s.Lang, salvia_config.NAVIGATION_RULES["set_feminicide"]),
		}, utils.GetFullHtmlFuncMap())
}

// FeminicidePUT maneja la solicitud PUT para actualizar o aprobar un caso de víctima.
// Según el parámetro "by" de la URL, decide si se aprueba el caso o se actualiza la información.
/*
func FeminicidePUT(c *gin.Context) {
	var res string
	var code int
	// Se obtiene la sesión y se extrae el ID de sesión.
	session := sessions.Default(c)
	...
	if by != "" && salvia_config.TranslateLocale(by, s.Lang) == "approve_case" {
		// Verifica el permiso "approve_feminicide".
		if !utils.CheckPermission(salvia_config.PermissionsByRole, "approve_feminicide", s.CurrentRole, c) {
			return
		}
		code, res = salvia_ctrl.ApproveFeminicide(id, &db.ConnData{}, dbClientConfig, dbServerConfig)

	} else if by != "" && salvia_config.TranslateLocale(by, s.Lang) == "update" {
		// Si "by" indica "update", se verifica el permiso "update_feminicide".
		if !utils.CheckPermission(salvia_config.PermissionsByRole, "update_feminicide", s.CurrentRole, c) {
			return
		}
		...
		buf := new(bytes.Buffer)
		buf.ReadFrom(c.Request.Body)
		//code, res := salvia_ctrl.CtrlSetFeminicide(buf.String())
		code, res = salvia_ctrl.UpdateFeminicide(buf.String(), id, s, dbClientConfig, dbServerConfig)

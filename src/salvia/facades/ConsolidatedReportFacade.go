...
// DownloadConsolidatedReport maneja la descarga del reporte Excel consolidado por rango de fechas
func DownloadConsolidatedReport(c *gin.Context, svc service.ReportService) {
	...
	// 1. Autorización: verificar permiso report_followups_consolidated
	if !utils.CheckPermission(salvia_config.PermissionsByRole, "report_followups_consolidated", s.CurrentRole, c) {
	...
	// Ajustar rango temporal para abarcar los días completos (00:00:00 del start y 23:59:59 del end)
	...
	file, err := svc.GenerateConsolidatedFollowUpsReport(c.Request.Context(), service.FollowUpsReportRange{
		StartDate: startRange,
		EndDate:   endRange,
	})
	if err != nil {
		if err.Error() == "no_cases_found" {
			c.JSON(http.StatusBadRequest, gin.H{"error_code": "VALIDATION_FAILED", "message": "No se encontraron casos creados en el rango de fechas seleccionado"})
			return
		}
	...
	// 5. Configurar headers HTTP para descarga del binario
	filename := fmt.Sprintf("reporte-casos_%s_%s.xlsx", req.StartDate, req.EndDate)
	c.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
	c.Header("Content-Disposition", "attachment; filename=\""+filename+"\"")
	c.Header("Cache-Control", "no-cache, no-store, must-revalidate")

	// Escribir archivo al stream de la respuesta

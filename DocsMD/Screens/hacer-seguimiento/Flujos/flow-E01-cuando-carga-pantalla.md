...
PASO 1 — Inicializar la app Vue
  ...
  → Se ejecuta mounted() → llama a loadFollowUp()
...
PASO 2 — Llamar al backend para cargar el seguimiento
  ...
  SI respuesta ok:
    → data = JSON parseado { followUp, victimInfo, caseStatus }
    → CONTINÚA PASO 3
...
PASO 10 — Responder al frontend

  200 {
    followUp:   { form_submission_id, case_id, status, completed_at, ... },
    victimInfo: { Names, LastNames, Phone, ... },
    caseStatus: "cd" | "ra" | "r" | ...
  }
...
|----------------------------------------------------------------------------|---------------|
| Qué pasa si agent_id no coincide con el profesional asignado al caso       | PASO 6        |
